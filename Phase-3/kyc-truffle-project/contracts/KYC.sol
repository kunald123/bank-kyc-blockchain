...
contract kyc {
    ...
    //  Struct data type of customer
    //  uname - username 
    //  dataHash - customer data hash
    //  rating - rating given to customer given based on regularity. This is also kyc Status.
    //  upvotes - number of upvotes recieved from banks
    //  downvotes - number of down votes recieved from banks
    ...
    //  complaintsReported - number of complaints raised for this bank 
    //  isAllowedToVote - whether this bank is allowed to vote or not
    //  regNumber - Bank registration number
    struct Bank {
        string name;
        address ethAddress;
        uint rating;
    ...
    //  Struct finalCustomer  is created to identify trusted customers.
    ...
    // Constructor for setting the admin as the person who deploys the smart contract onto the network.
    constructor() public {
        admin = msg.sender;
    }
    
    //ADMIN INTERFACE to add and remove a bank
    ...
    function addBank(string memory bankName, address bankAddress, string memory bankRegistrationNumber) public payable returns(bool) {
        ...
        if(admin==msg.sender) {
            ...
            allBanks[allBanks.length - 1] = Bank(bankName, bankAddress, 0, 0, 0,false,bankRegistrationNumber);
    ...
    // Function is used by the admin to turn off isAllowedToVote
    // You need to verify if the user trying to call this function is admin or not.
    // @param bankAddress - address of the bank
    
    function disableIsAllowedToVote(address bankAddress) public payable {
        
        
        if(admin==msg.sender) {
            for(uint i = 0; i < allBanks.length; ++ i) {
                if(allBanks[i].ethAddress == bankAddress) {
                    allBanks[i].isAllowedToVote=false;
                }
            }
        }
    ...
    function addCustomer(string memory userName, string memory dataHash) public payable returns(uint) {
        ...
        for(uint i = 0; i < allRequests.length; ++i) {
            if(stringsEquals(allRequests[i].uname, userName) && allRequests[i].bankAddress == msg.sender && allRequests[i].isAllowed && stringsEquals(allRequests[i].dataHash,dataHash)) {
                ...
                allCustomers[allCustomers.length-1] = Customer(userName, dataHash, 0, 0, 0,msg.sender, "0");
                return 1;
            }
        }
    ...
    // Function is used to fetch bank complains.
    // @param bankAddress is passed as address to fetch bank ratings.
    // @returns ratings as unsigned integer.
    function getBankComplaints(address bankAddress) public payable returns(uint) {
        for(uint i = 0; i < allBanks.length; ++ i) {
            if(allBanks[i].ethAddress == bankAddress) {
                return allBanks[i].complaintsReported;
            }
        }
        return 0;
    }
        ...
        for(uint i = 0; i < allCustomers.length; ++ i) {
            if(stringsEquals(allCustomers[i].uname, userName)) {
                    ...
                    if(allCustomers[i].rating*10 >= 0.5*10){
                        ...
                        allFinalCustomers[allFinalCustomers.length-1]=FinalCustomer(allCustomers[i].uname,allCustomers[i].dataHash,allCustomers[i].rating,allCustomers[i].upvotes,0,allCustomers[i].bank,allCustomers[i].password);
                    }
                return 1;
            }
        }
